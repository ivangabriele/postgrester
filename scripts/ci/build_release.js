...
function buildUnsignedArchive() {
  return new Promise(resolve => {
    ...
    archiveOutput.on('close', () => {
      ß.info(`Unsigned Archive size: ${numeral(archive.pointer()).format('0.00 b')}.`)
      resolve()
    })
    ...
    archiveOutput.on('end', () => ß.warn('Warning: Data has been drained'))

    archive.on('error', err => {
      ß.error(`Error: ${err.message}`)
      process.exit(1)
    })
    archive.on('warning', err => {
      if (err.code === 'ENOENT') {
        ß.warn(`Warning: ${err.message}`)
      } else {
        ß.error(`Error: ${err.message}`)
        process.exit(1)
      }
